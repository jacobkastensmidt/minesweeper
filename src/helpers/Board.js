 ...
 * @param {number} height
 */
export const getTileRowAndColumn = (index, height) => {
...
export const getTranslatedTileIndex = (index, translation, height, width) => {
  const {row, column} = getTileRowAndColumn(index, height);
 ...
 */
export const generateTileData = (height, width, customNumberOfMines = null) => {
  const numberOfMines = customNumberOfMines || determineNumberOfMines(height, width);
